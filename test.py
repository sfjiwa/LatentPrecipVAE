...
for x,y1 in zip(precip,slp):
    ...
    if(torch.mean((x-x_hat)**2)<0.5):
        ...
        plt.gca().invert_yaxis()
        plt.show()
        plt.imshow(x_lin[0][0].cpu().detach().numpy())
        plt.gca().invert_yaxis()
        plt.show()
        plt.imshow(z[0][0].cpu().detach().numpy())
        plt.gca().invert_yaxis()
        plt.show()
        plt.imshow(z_h[0][0].cpu().detach().numpy())
        plt.gca().invert_yaxis()
        plt.show()
...
""" 
    if(1):
        ...
        plt.colorbar(shrink=0.9)
        plt.show()
        plt.imshow(x_hat[0][0].detach().cpu().numpy()+x_lin[0][0].detach().cpu().numpy(),vmin=0,vmax=8)
        plt.gca().invert_yaxis()
        plt.title('vae resconstruction')
        plt.colorbar(shrink=0.9)
        plt.show()
